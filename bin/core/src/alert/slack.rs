...
pub async fn send_alert(
  ...
  // A template replaces the Block Kit layout with its own text: an
  // admin writing a message is choosing the words, and rebuilding
  // blocks around arbitrary text would only guess at structure.
  let (text, blocks): (_, Option<_>) = if let Some(template) =
    template
  {
    ...
    match &alert.data {
      ...
      AlertData::ProcedureFailed { id, name } => {
        let text =
          format!("{level} | Procedure *{name}* has *failed*");
        let blocks = vec![
          Block::header(text.clone()),
          Block::section(resource_link(
